...
applicants_lm <- lm(Applicants ~ Year, data = marathon_data)
starters_lm <- lm(Starters ~ Year, data = marathon_data)
...
dropout_2020 <- marathon_data %>% filter(Year == 2020) %>% pull(Dropout_Rate)

# Calculate average dropout rate without 2020
dropout_avg <- marathon_data %>% filter(Year != 2020) %>% summarise(mean = mean(Dropout_Rate, na.rm = TRUE)) %>% pull(mean)
```
...
winners <- winners %>% mutate(Time_hours = Time * 24)
men_lm <- lm(Time_hours ~ Year, data = winners %>% filter(Category == "Men"))
women_lm <- lm(Time_hours ~ Year, data = winners %>% filter(Category == "Women"))
wheelchair_men_lm <- lm(Time_hours ~ Year, data = winners %>% filter(Category == "Wheelchair Men"))
wheelchair_women_lm <- lm(Time_hours ~ Year, data = winners %>% filter(Category == "Wheelchair Women"))
...
```{r}
cor_test <- cor.test(marathon_data$Starters, marathon_data$Raised, use = "complete.obs")
cor_test$estimate
cor_test$p.value   
```
